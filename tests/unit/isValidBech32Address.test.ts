...
describe('isValidBech32Address', () => {
  ...
  test.each(validBech32Addresses)(
    'should return true for valid Bech32 address: %s',
    (address: string) => {
      expect(isValidBech32Address(address)).toBe(true);
    }
  );

  test.each(invalidBech32Addresses)(
    'should return false for invalid Bech32 address: %s',
