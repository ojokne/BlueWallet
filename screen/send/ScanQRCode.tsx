...
import { hexToUint8Array, uint8ArrayToBase64, uint8ArrayToHex, uint8ArrayToString } from '../../blue_modules/uint8array-extras/index';
  ...
  const _onReadUniformResource = (ur: string) => {
    try {
      ...
      if (Object.values(animatedQRCodeData).length === total) {
        ...
        const payloadBytes = hexToUint8Array(String(payload));
        if (uint8ArrayToString(payloadBytes).startsWith('psbt')) {
          // its a psbt, and whoever requested it expects it encoded in base64
          data = uint8ArrayToBase64(payloadBytes);
        } else {
          // its something else. probably plain text is expected
          data = uint8ArrayToString(payloadBytes);
        }
        if (launchedBy) {
          const merge = true;
          const popToAction = StackActions.popTo(launchedBy, { onBarScanned: data }, { merge });
